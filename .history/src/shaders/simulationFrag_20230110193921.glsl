...
uniform sampler2D positionsB; // Data Texture containing original positions
uniform float uTime;
...
void main() {
  ...
  // pos.z += tan(length(length(pos.xy) * 10.0) - t) * 1.0;
      vec3 n = vec3(snoise(vec4(0.08*pos2,2.05*time*0.1)));
    n*=vec3(snoise(vec4(0.12*pos+4.7,time*0.3)));
  ...
  finalPos = mix(finalPos, bTex, timer);
  gl_FragColor = vec4(pos2, 1.0);
}